# lingya_agents_sdk.WorkspaceApi

All URIs are relative to *https://tenant.example.com*

Method | HTTP request | Description
------------- | ------------- | -------------
[**get_workspace_file_preview**](WorkspaceApi.md#get_workspace_file_preview) | **GET** /api/agents/channel/openapi/v1/{channelId}/chat/conversations/{conversationId}/workspace/files/preview | 创建工作区文件预览地址 / Create a workspace file preview URL
[**list_workspace_artifacts**](WorkspaceApi.md#list_workspace_artifacts) | **GET** /api/agents/channel/openapi/v1/{channelId}/chat/conversations/{conversationId}/workspace/files | 分页查询工作区制品 / List workspace artifacts


# **get_workspace_file_preview**
> PreSignedReadUrl get_workspace_file_preview(channel_id, conversation_id, path)
...
with lingya_agents_sdk.ApiClient(configuration) as api_client:
    ...
    api_instance = lingya_agents_sdk.WorkspaceApi(api_client)
    ...
    path = 'path_example' # str | 工作区相对文件路径。 / Workspace-relative file path.

    try:
        # 创建工作区文件预览地址 / Create a workspace file preview URL
        api_response = api_instance.get_workspace_file_preview(channel_id, conversation_id, path)
        print("The response of WorkspaceApi->get_workspace_file_preview:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling WorkspaceApi->get_workspace_file_preview: %s\n" % e)
```
 ...
 **path** | **str**| 工作区相对文件路径。 / Workspace-relative file path. | 
...
> WorkspaceArtifactList list_workspace_artifacts(channel_id, conversation_id, current=current, size=size, order_by=order_by, order_direction=order_direction, order_null_handling=order_null_handling, keyword=keyword, prefix=prefix)
...
with lingya_agents_sdk.ApiClient(configuration) as api_client:
    ...
    prefix = 'prefix_example' # str | 工作区相对路径前缀。 / Workspace-relative path prefix. (optional)

    try:
        # 分页查询工作区制品 / List workspace artifacts
        api_response = api_instance.list_workspace_artifacts(channel_id, conversation_id, current=current, size=size, order_by=order_by, order_direction=order_direction, order_null_handling=order_null_handling, keyword=keyword, prefix=prefix)
        print("The response of WorkspaceApi->list_workspace_artifacts:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling WorkspaceApi->list_workspace_artifacts: %s\n" % e)
```
 ...
 **prefix** | **str**| 工作区相对路径前缀。 / Workspace-relative path prefix. | [optional] 
