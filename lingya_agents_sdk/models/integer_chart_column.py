...
class IntegerChartColumn(BaseModel):
    """
    ...
    values: List[Optional[StrictInt]] = Field(description="字段 values / values field。")
    __properties: ClassVar[List[str]] = ["name", "type", "values"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['INTEGER', 'LONG']):
            raise ValueError("must be one of enum values ('INTEGER', 'LONG')")
        return value
