...
class InstantChartColumn(BaseModel):
    """
    ...
    values: List[Optional[datetime]] = Field(description="字段 values / values field。")
    __properties: ClassVar[List[str]] = ["name", "type", "values"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['INSTANT']):
            raise ValueError("must be one of enum values ('INSTANT')")
        return value
