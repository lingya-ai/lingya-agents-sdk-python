...
class LocalDateChartColumn(BaseModel):
    """
    ...
    values: List[Optional[date]] = Field(description="字段 values / values field。")
    __properties: ClassVar[List[str]] = ["name", "type", "values"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['LOCAL_DATE']):
            raise ValueError("must be one of enum values ('LOCAL_DATE')")
        return value
